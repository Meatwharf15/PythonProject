def place_order(menu):
    ...
    while True:
        ...
        makeOrder = input("Would you like to order something? (y/n)")
        if makeOrder == "y":
            print(f"Alright. What would you like to order?")
        else:
            break
        ...
        for key, meal in meals.items():
            ...
            for option, price in meal.items():
                # Print the menu item number, food category, meal, and price
                print_menu_line(i, key, option, price)

                # Update the menu selection number
                i += 1

        # TODO: Ask customer to input menu item number
        menu_selection = input("Which menu item number would you like to order?")
...
def update_order(order, menu_selection, menu_items):
    ...
    if menu_selection.isdigit():
        ...
        if 1 <= menu_selection <= len(menu_items):
            ...
            order_list = {
                "Item name": ordered_item.get("Item name"),
                "Price": ordered_item.get("Price"),
                "Quantity": quantity
            }
